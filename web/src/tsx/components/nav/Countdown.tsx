...
export default function Countdown({nav}: CountdownProps) {
  ...
  return (
    <div
      ...
      {Math.floor(timeRemaining / (1000 * 60 * 60 * 24)) >= 0 ? <span className="text-2xl xl:text-2xl  my-auto mx-2">
        {('0' + Math.floor(timeRemaining / (1000 * 60 * 60 * 24))).slice(
          Math.floor(timeRemaining / (1000 * 60 * 60 * 24)).toString().length >
            2
            ? -3
            : -2,
        )}{' '}
        : {('0' + Math.floor(timeRemaining / (1000 * 60 * 60))).slice(-2)} :{' '}
        {('0' + Math.floor(timeRemaining / (1000 * 60))).slice(-2)} :{' '}
        {('0' + Math.floor(timeRemaining / 1000)).slice(-2)}
      </span> : '00 : 00 : 00 : 00'}
      {/* <CountdownTime
