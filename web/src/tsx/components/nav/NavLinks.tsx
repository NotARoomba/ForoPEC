...
export default function NavLinks() {
  ...
  return (
    <div
      ...
      className="h-11 relative bg-pastel-light-blue w-5/12 ml-auto mr-5 my-auto rounded-3xl justify-around flex shadow-inner-lg">
  ...
  stiffness: 50
      ...
      <motion.div
        className=" z-[5] left-0 absolute bg-pastel-dark-blue h-11 rounded-3xl shadow-xl"
        transition={{
          type: 'spring',
          damping: 10,
          stiffness: 50,
      ...
      {menuItems.map((v, i) => (
        <Link
          onMouseEnter={e => {
            setMPos(e.currentTarget.offsetLeft - 32);
            setMWidth(e.currentTarget.clientWidth + 64);
          }}
          key={i}
          to={v.toLowerCase() == 'inicio' ? '/' : v.toLowerCase()}
          onClick={e => updatePos(e)}
          className=" my-auto z-20 text-2xl font-bold">
          {v}
        </Link>
      ))}
    </div>
